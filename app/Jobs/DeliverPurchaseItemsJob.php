...
class DeliverPurchaseItemsJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        // Две схемы выдачи:
        //  - Привилегии/рейты/изучения (is_command=1) → RCON-команда (таблица shopping)
        //    на ВЫБРАННЫЙ сервер, СРАЗУ, даже если игрок оффлайн. Это критично для
        //    пропуска очереди (queue-skip): пропуск должен быть активен ДО захода.
        //  - Обычные товары → bucket, плагин выдаёт по short_name на любом сервере.
        if ($item->is_command) {
    ...
    protected function deliverViaRcon(ShopItem $item): void
    {
        ...
        // Привилегия выдаётся RCON-командой на ВЫБРАННЫЙ при покупке сервер (выбор
        // сервера обязателен для is_command). На каждом сервере свои группы/права
        // (IQPermissions хранит их по-серверно), поэтому отправляем только на выбранный,
        // а не на все — иначе одна покупка активировала бы привилегию сразу везде.
        Shopping::create([
