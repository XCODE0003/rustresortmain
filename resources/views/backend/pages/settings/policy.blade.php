                                ...
                                <form action="{{ route('backend.settings') }}" method="POST" enctype="multipart/form-data">
                                    ...
                                    @foreach(getlangs() as $key => $value)
                                        ...
                                        <div class="row g-4">
                                            <div class="col-lg-12">
                                                <div class="form-group">
                                                    <label class="form-label" for="policy_{{ $key }}">{{ __('ПОЛИТИКА КОНФИДЕНЦИАЛЬНОСТИ') }}</label>
                                                    <div class="form-control-wrap">
                                                <textarea type="text" class="form-control"
                                                          id="policy_{{ $key }}" name="setting_policy_{{ $key }}">{{ config('options.policy_'.$key) }}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row g-4">
                                            <div class="col-lg-12">
                                                <div class="form-group">
                                                    <label class="form-label" for="term_{{ $key }}">{{ __('УСЛОВИЯ ОБСЛУЖИВАНИЯ') }}</label>
                                                    <div class="form-control-wrap">
                                                <textarea type="text" class="form-control"
                                                          id="term_{{ $key }}" name="setting_term_{{ $key }}">{{ config('options.term_'.$key) }}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
