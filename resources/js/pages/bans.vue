    ...
    <div class="container flex flex-col gap-6 md:gap-8">
        ...
        <form @submit.prevent="submitSearch" class="flex items-center gap-2">
            <input
                v-model="searchQuery"
                type="text"
                :placeholder="$t('bans_page.search_placeholder')"
                class="w-full rounded-lg border border-StrokeGray bg-white/4 px-4 py-2.5 text-sm text-white placeholder-TextGray outline-none transition-colors focus:border-white/40"
            />
            <button
                type="submit"
                class="shrink-0 rounded-lg border border-StrokeGray px-5 py-2.5 text-sm font-medium text-TextGray transition-colors hover:border-Orange hover:text-Orange"
            >
                {{ $t('bans_page.search_label') }}
            </button>
        </form>
        ...
        <div v-else class="flex flex-col gap-3">
            ...
            >
                <table
                    class="w-full min-w-[800px] border-collapse text-left text-[13px] text-white"
                >
                    <thead>
                        <tr
                    ...
                    <tbody class="divide-y divide-StrokeGray">
                        ...
                        >
                            ...
                            <td class="whitespace-nowrap px-3 py-2 text-xs sm:px-4 sm:py-2.5">
                                <span v-if="ban.server_names && ban.server_names.length > 0" class="text-white/70">
                                    {{ ban.server_names.length >= Object.keys(servers).length && Object.keys(servers).length > 1
                                        ? $t('bans_page.all_servers')
                                        : ban.server_names.join(', ') }}
                                </span>
                                <span v-else class="text-TextGray/40">—</span>
                            </td>
                            <td
        ...
        <div
            ...
            class="flex items-center justify-center gap-2 border-t border-StrokeGray/80 pt-4"
        >
            <Link
...
function submitSearch(): void {
    router.get('/bans', { search: searchQuery.value, page: 1 }, { preserveState: true, replace: true });
}
