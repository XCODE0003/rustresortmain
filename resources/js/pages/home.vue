                            ...
                            <div class="flex w-full flex-col justify-between gap-3">
                                ...
                                <div class="flex items-center gap-3">
                                    <button
                                        ...
                                        class="inline-flex cursor-pointer items-center gap-1.5 rounded-md bg-Orange px-5 py-2.5 text-[12px]/[12px] font-medium text-black transition-colors duration-300 hover:bg-Orange/80"
                                    >
                                    ...
                                    <span
                                        v-else
                                        class="inline-flex cursor-not-allowed items-center justify-center rounded-md bg-Orange/35 px-5 py-2.5 text-[12px]/[12px] font-medium text-black/55"
                                        aria-disabled="true"
                                    >
                                        {{ $t('server.connect') }}
                                    </span>
            ...
            <div class="home-shop-section flex w-full flex-col gap-6" style="opacity: 0">
                ...
                <div class="flex w-full flex-col gap-8">
                    ...
                    >
                        ...
                        <button
                            v-for="category in visibleShopCategories"
                            :key="category.id"
                            type="button"
...
// Только категории с товарами — пустые (например удалённые «Рецепты») не показываем.
const visibleShopCategories = computed(() => {
    const usedCategoryIds = new Set(props.shopItems.map((item) => item.category_id));
    return props.shopCategories.filter((category) => usedCategoryIds.has(category.id));
});
...
// Коннект-строка для копирования: домен из админки (options.connect) или IP.
